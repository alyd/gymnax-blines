...
import sys
sys.path.insert(1, './shaped_envs/')

from my_continuous_mountain_car import MyContinuousMountainCar
from my_mountain_car import MyMountainCar
...
class RolloutManager(object):
    def __init__(self, model, env_name, env_kwargs, env_params):
        ...
        if env_name == 'MyContinuousMountainCar':
            self.env = MyContinuousMountainCar(**env_kwargs)
            self.env_params = self.env.default_params
        elif env_name == 'MyMountainCar':
            self.env = MyMountainCar(**env_kwargs)
            self.env_params = self.env.default_params
        else:
    ...
    @partial(jax.jit, static_argnums=0)
    def batch_step(self, keys, state, action, max_reached_pos, goal_return):
        next_obs, next_state, reward, done, _ = jax.vmap(self.env.step, in_axes=(0, 0, 0, None, None, None))(keys, state, action, max_reached_pos, goal_return, self.env_params)
        return next_obs, next_state, reward, done, _

    @partial(jax.jit, static_argnums=(0, 3))
    def batch_evaluate(self, rng_input, train_state, num_envs, max_reached_pos, goal_return):
        ...
        def policy_step(state_input, _):
            ...
            obs, state, train_state, rng, cum_reward, cum_real_reward, cum_goal_reward, cum_shape_reward, valid_mask = state_input
            rng, rng_step, rng_net = jax.random.split(rng, 3)
            action, _, _, rng = self.select_action(train_state, obs, rng_net)
            next_o, next_s, reward, done, info = self.batch_step(
            ...
            new_cum_real_reward = cum_real_reward + info["real_reward"] * valid_mask
            new_cum_goal_reward = cum_goal_reward + info["goal_reward"] * valid_mask
            new_cum_shape_reward = cum_shape_reward + info["shape_reward"] * valid_mask
        ...
        carry_out, scan_out = jax.lax.scan(
            ...
            [
                ...
                train_state,
                rng_episode,
                jnp.array(num_envs * [0.0]), # total training reward
                jnp.array(num_envs * [0.0]), # real reward
                jnp.array(num_envs * [0.0]), # goal reward
                jnp.array(num_envs * [0.0]), # shape reward
        ...
        cum_real_return = carry_out[-4].squeeze()
        cum_goal_return = carry_out[-3].squeeze()
        cum_shape_return = carry_out[-2].squeeze()
        return jnp.mean(cum_return), jnp.mean(cum_real_return), jnp.mean(cum_goal_return), jnp.mean(cum_shape_return)
...
def train_ppo(rng, config, model, params, mle_log):
    ...
    def get_transition(
        ...
        next_obs, next_state, reward, done, _ = rollout_manager.batch_step(
            b_rng, state, action, max_reached_pos, goal_return
        )
        max_reached_pos = jnp.maximum(jnp.max(jnp.abs(-0.5-next_obs[:,0])),max_reached_pos)
       
        batch = batch_manager.append(
            batch, obs, action, reward, done, log_pi, value
        )
        return train_state, next_obs, next_state, batch, new_key, max_reached_pos
    ...
    total_steps = 0
    goal_rewards = 0.0
    log_steps, log_return, log_real_return, log_goal_return, log_shape_return = [], [], [], [], []
    t = tqdm.tqdm(range(1, num_total_epochs + 1), desc="PPO", leave=True)
    for step in t:
        train_state, obs, state, batch, rng_step, max_reached_pos = get_transition(
            train_state,
            obs,
            state,
            batch,
        ...
        if (step + 1) % config.evaluate_every_epochs == 0:
            ...
            rewards, real_rewards, goal_rewards, shape_rewards = rollout_manager.batch_evaluate(
                rng_eval,
                train_state,
            ...
            log_real_return.append(real_rewards)
            log_goal_return.append(goal_rewards)
            log_shape_return.append(shape_rewards)
            ...
            if mle_log is not None:
                mle_log.update(
                    ...
                    {"return": rewards},
                    {"real_return": real_rewards},
                    {"shape_return": shape_rewards},
                    model=train_state.params,
                    save=True,
                )
            wandb.log({"num_steps": total_steps, 
                       "return": rewards,
                       "real_return": real_rewards,
                       "goal_return": goal_rewards,
                        "shape_return": shape_rewards,
                        "max_reached_pos": max_reached_pos})
